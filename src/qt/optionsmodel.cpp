...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRoverAmount"))
        settings.setValue("nAnonymizeRoverAmount", 1000);
    nAnonymizeRoverAmount = settings.value("nAnonymizeRoverAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeRoverAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeRoverAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeRoverAmount:
            return QVariant(nAnonymizeRoverAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeRoverAmount:
            nAnonymizeRoverAmount = value.toInt();
            settings.setValue("nAnonymizeRoverAmount", nAnonymizeRoverAmount);
            emit AnonymizeRoverAmountChanged(nAnonymizeRoverAmount);
            break;
        default:
            break;
        }
