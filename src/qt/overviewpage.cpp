...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeRoverAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeRoverAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeRoverAmount = strAnonymizeRoverAmount.remove(strAnonymizeRoverAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRoverAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeRoverAmount*COIN) nMaxToAnonymize = nAnonymizeRoverAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeRoverAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeRoverAmount));
        strAnonymizeRoverAmount = strAnonymizeRoverAmount.remove(strAnonymizeRoverAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRoverAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
